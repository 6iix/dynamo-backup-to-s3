...
DynamoRestore.prototype.run = async function(finishCallback) {
  this._validateS3Backup(this.options);
  await this._validateTable(this.options);
  this._startDownload();
...
DynamoRestore.prototype._validateTable = async function(options) {
  var dynamodb = this.dynamodb;
  if (!options.table) {
  ...
  try {
    data = await dynamodb.listTables({}).promise();
    tableNames = [...data.TableNames];
    console.log(tableNames.length);
    do {
      console.log('Starting loop');
      data = await dynamodb.listTables({ ExclusiveStartTableName: data.LastEvaluatedTableName }).promise();
      tableNames = [...tableNames, ...data.TableNames];
    } while (data.TableNames > 0);
  } catch (error) {
    this._checkTableExists(error);
  }
  console.log(tableNames.length);
  this._checkTableExists(null, { TableNames: tableNames });
};
...
DynamoRestore.prototype._checkTableExists = function(error, data) {
  ...
  if (data.TableNames.indexOf(this.options.table) > -1) {
    ...
    if (this.options.overwrite) {
      ...
      setTimeout(
        dynamodb.describeTable.bind(dynamodb, { TableName: this.options.table }, this._checkTableReady.bind(this)),
        1000
      );
    } else {
    ...
    function(error, meta) {
      if (error) {
        if (error.code === 'NotFound')
          this.emit('error', util.format('Could not find file in s3. %s', this.options.source));
...
DynamoRestore.prototype._extractSchema = function(template) {
  ...
  if (partitionkey) {
    ...
    if (!template[partitionkey]) {
      return this.emit(
        'error',
        util.format('Fatal Error. The --partitionkey "%s" you provided is not a valid column.', partitionkey)
      );
    }
  } else {
...
DynamoRestore.prototype._createTable = function(callback) {
  ...
  dynamodb.createTable(
    ...
    function(error, data) {
      ...
      setTimeout(
        dynamodb.describeTable.bind(dynamodb, { TableName: data.TableName }, this._checkTableReady.bind(this)),
        5000
      );
    }.bind(this)
  );
...
DynamoRestore.prototype._checkTableReady = function(error, data) {
  ...
  if (data && data.Table && data.Table.TableStatus === 'ACTIVE') {
    ...
    setTimeout(
      dynamodb.describeTable.bind(dynamodb, { TableName: data.Table.TableName }, this._checkTableReady.bind(this)),
      1000
    );
  }
};
